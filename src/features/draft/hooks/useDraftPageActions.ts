...
export function useDraftPageActions(drafts: QuizQuestion[], deleteDraft: any, sendDraftToTelegram: any, telegram: any, moveManyToDraft: any) {
  ...
  const handleSend = async (draft: QuizQuestion) => {
    if (!draft.topic) {
      alert('পোল পাঠানোর আগে অবশ্যই একটি বিষয় (Topic) নির্বাচন করতে হবে।');
      return;
    }
  ...
  const handleBulkSend = async () => {
    ...
    const missingTopic = selectedDrafts.find(d => !d.topic);
    if (missingTopic) {
      alert('পোল পাঠানোর আগে সবগুলো পোল নির্বাচন করে বিষয় (Topic) সেট করুন।');
      return;
    }
  ...
  const handleSetTopic = async (topic: string) => {
    ...
    await moveManyToDraft(selectedDrafts.map(d => ({ ...d, topic })));
    setSelectedIds(new Set());
  };
